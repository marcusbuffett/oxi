...
- **Global Scalars** (`NUM_GLOBALS = 6`): Normalized self/opponent remaining time, ratios vs base time, move count, and player Elo (see `GlobalFeatures::to_feature_vector` in `src/inference.rs`).
...
- **Absolute + Relative Positioning**: Learned absolute embeddings (`pos_embed`) complement Shaw 2D relative attention over 15×15 displacement buckets (`src/shaw.rs`) with per-head shared embeddings and learnable scale parameters. Grouped-query attention is supported via `num_kv_heads`.
...
- **Scheduler**: `CosineAnnealingWithWarmup` (10% warmup default when unspecified) drives three LR tiers between `lr_max` and `lr_min`.
...
- `lr_max = 1e-4`, `lr_min = 1e-6`, `lr_scalar = 100.0`.
- `policy_loss_weight = 0.15`, `value_loss_weight = 0.15`, `time_usage_loss_weight = 0.008`.
- `gradient_clip = 1.0`, `weight_decay = 1e-5`, `warmup_steps` defaults to 10% of total iterations when omitted.

#### Recent Training Snapshot
- Configuration: 14 layers, `embed_dim = 512`, `num_heads = 8`, effective batch size 8192 via accumulation.
- Schedule: `lr_max = 3e-3` with 10% warmup across 23k iterations, cosine annealing thereafter.
...
- Evaluate grouped-query attention (`num_kv_heads`) for further memory savings.
