...
use crate::moves::{decode_move_spatial, encode_move_spatial, get_side_info};
    ...
    B::IntElem: From<i32>,
{
    ...
    pub fn predict(
        ...
        let board_encoded = tensor.reshape([16, 8, 8]).unsqueeze();
        ...
        for legal_move in pos.legal_moves() {
            let uci = legal_move
                ...
                .to_string();
            if let Some((from_idx, to_idx)) = encode_move_spatial(&uci) {
    ...
    pub fn predict_batch(
        ...
        for (fen, elo_self, elo_oppo) in positions {
            ...
            let board = tensor.reshape([16, 8, 8]).unsqueeze();
