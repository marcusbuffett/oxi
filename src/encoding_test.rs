...
mod tests {
    ...
    fn test_encode_starting_position() {
        ...
        let pos: Chess = parsed_fen.into_position(shakmaty::CastlingMode::Standard).unwrap();
        
        let encoded = encode_position(&pos);
        
        // Should have 16 planes * 64 squares = 1024 values
        ...
        let plane_names = [
            ...
            "En Passant", "Move Count", "Castling Rights", "Is Check"
        ];
    ...
    fn test_encode_position_with_en_passant() {
        ...
        let en_passant_count: f32 = (0..64)
            .map(|i| encoded[en_passant_plane_start + i])
            .sum();
    ...
    fn test_encode_position_castling_rights() {
        ...
        let castling_count: f32 = (0..64)
            .map(|i| encoded[castling_plane_start + i])
            .sum();
    ...
    fn test_encode_position_in_check() {
        ...
        let check_sum: f32 = (0..64)
            .map(|i| encoded[is_check_plane_start + i])
            .sum();
