...
//     assert!(
//         (sigma_policy - 1.0).abs() < 1e-6,
//         "Initial policy sigma should be 1.0, got {}",
//         sigma_policy
//     );
//     assert!(
//         (sigma_value - 1.0).abs() < 1e-6,
//         "Initial value sigma should be 1.0, got {}",
//         sigma_value
//     );
//     assert!(
//         (sigma_side_info - 1.0).abs() < 1e-6,
//         "Initial side info sigma should be 1.0, got {}",
//         sigma_side_info
//     );
//
...
//
//     let elo_self: Tensor<Backend, 2, Int> =
//         Tensor::from_data([[elo_bin_1500], [elo_bin_1600]], &device);
//     let elo_oppo: Tensor<Backend, 2, Int> =
//         Tensor::from_data([[elo_bin_1500], [elo_bin_1600]], &device);
...
//     let fens =
//         vec!["rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1".to_string(); batch_size];
//     let global_features: Tensor<Backend, 2, Int> = Tensor::zeros([batch_size, 1], &device);
//
...
//     assert!(
//         output.uncertainties.is_some(),
//         "Output should include uncertainties"
//     );
//     let (out_sigma_policy, out_sigma_value, out_sigma_side_info) = output.uncertainties.unwrap();
//     assert!(
//         (out_sigma_policy - 1.0).abs() < 1e-6,
//         "Output policy sigma should be 1.0"
//     );
//     assert!(
//         (out_sigma_value - 1.0).abs() < 1e-6,
//         "Output value sigma should be 1.0"
//     );
//     assert!(
//         (out_sigma_side_info - 1.0).abs() < 1e-6,
//         "Output side info sigma should be 1.0"
//     );
//
